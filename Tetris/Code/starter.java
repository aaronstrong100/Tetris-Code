...
public class starter implements InputControl, InputKeyControl {
	...
	public static void scoreBoard(){
		...
		try{
			File scoreHolder = new File("scoreHolder.txt");
		}catch(Exception e){}
		try 
		{
			FileWriter myWriter = new FileWriter("scoreHolder.txt");
			myWriter.write(name + ";" + score + "\n");
			myWriter.close();
		} catch (IOException e){}
		EasyReader scoreReader = new EasyReader("leaderboard.txt");
		int lineNumber = 0;
		...
		int[] scores = new int[lineNumber+1];
		String[] names = new String[lineNumber+1];
		scoreReader = new EasyReader("leaderboard.txt");
		for(int i = 0; i<lineNumber; i++)
		{
			String temp = scoreReader.readLine();
			if(temp.indexOf(";")>-1)
			{
				names[i] = temp.substring(0,temp.indexOf(";"));
				scores[i] = Integer.parseInt(temp.substring(temp.indexOf(";")+1));
			}
		}
		names[lineNumber] = name;
		...
		if(names.length>10)
		{
			topScorers = new Text[10];
			topScores = new Text[10];
		}
		else
		{
			topScorers = new Text[names.length];
			topScores = new Text[names.length];
		}
